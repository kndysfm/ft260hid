...
impl<'a> I2c<'a> {
  ...
  pub fn read(&self, addr: u8, flag: Flag, buf: &mut [u8], len: usize, wait_timer: u32) -> Ft260Result<usize> {
    assert!(self.inited);
    reports::ft260_i2c_master_read(self.device, addr, Self::flag_to_cond(flag), buf, len, wait_timer)
  }
  ...
  pub fn write_read(&self, addr: u8, buf_write: &[u8], len_write: usize, buf_read: &mut [u8], len_read: usize, wait_timer: u32) -> Ft260Result<()> {
    assert!(self.inited);
    ...
    match self.read(addr, Flag::ReStartAndStop, buf_read, len_read, wait_timer) {
      Ok(bytes_read) => {
